        ...
        process: [{
			...
			load: {
				...
				filter: function(record) {
					...
					if(!hasPovertySignature(baseRecord)) {
						...
						return false;
					} else if(xelib.EditorID(baseRecord).substring(0, 5) == "Dummy") {
						return false;
			...
			patch: function (record, helpers, settings, locals) {
				if(settings.processREFR) {
					...
					if(!xelib.HasElement(patchFile, "LVLI\\p" + editorID)) {
						lvliRecord = AddPovertyLVLI(patchFile, baseRecord, "REFR");
					} else {
						lvliRecord = xelib.GetElement(0, xelib.Path(patchFile) + "\\LVLI\\p" + editorID);
					}
					...
					switch(xelib.Signature(baseRecord)) {
						case "ALCH":
							...
							} else if(xelib.GetHexFormID(xelib.GetLinksTo(baseRecord, "ENIT\\Sound - Consume"), true, false) == "000B6435") {
								xelib.SetValue(record, "NAME", "DummyDrink");
							} else {
						...
						case "INGR":
							...
							break;
						case "MISC":
							if(xelib.EditorID(lvliRecord) == "Gold001") {
			...
			patch: function (record, helpers, settings, locals) {
				if(settings.processCONT) {
					...
					//Cycle through items
					for(i = 0; i < xelib.ElementCount(xelib.GetElement(record, "Items")); i++) {
						let item = xelib.GetLinksTo(record, "Items\\[" + j.toString() + "]\\CNTO\\Item");
						let editorID = xelib.EditorID(item);
						
						//Replace items with poverty LVLI
						if(("LVLI|KEYM".includes(xelib.Signature(item))) || (editorID.includes("Dummy")) || isInBlacklist(locals.blacklist, editorID)) {
							j++;
						} else {
							let lvliRecord;
							if(!xelib.HasElement(patchFile, "LVLI\\p" + editorID)) {
								lvliRecord = AddPovertyLVLI(patchFile, xelib.GetWinningOverride(item), "CONT");
							} else {
								lvliRecord = xelib.GetElement(0, xelib.Path(patchFile) + "\\LVLI\\p" + editorID);
							}
							xelib.SetLinksTo(record, lvliRecord, "Items\\[" + j.toString() + "]\\CNTO\\Item");
						}
					}
				}
			}
			...
			load: {
				...
				overrides: true,
				filter: function(record) {
					if(locals.blacklistLVLI.includes(xelib.EditorID(record))) {
						return false;
			...
			patch: function (record, helpers, settings, locals) {
				if(settings.processLVLI) {
					...
					for(i = 0; i < xelib.ElementCount(xelib.GetElement(record, "Leveled List Entries")); i++) {
						let leveledlist = xelib.GetLinksTo(record, "Leveled List Entries\\[" + j.toString() + "]\\LVLO\\Reference");
						let editorID = xelib.EditorID(leveledlist);
						
						//Replace leveled entry with poverty LVLI
						if(("LVLI|KEYM".includes(xelib.Signature(leveledlist))) || (editorID.includes("Dummy")) || isInBlacklist(locals.blacklist, editorID)) {
							j++;
						} else {
							let lvliRecord;
							if(!xelib.HasElement(patchFile, "LVLI\\p" + editorID)) {
								lvliRecord = AddPovertyLVLI(patchFile, xelib.GetWinningOverride(leveledlist), "LVLI");
							} else {
								lvliRecord = xelib.GetElement(0, xelib.Path(patchFile) + "\\LVLI\\p" + editorID);
							}
							xelib.SetLinksTo(record, lvliRecord, "Leveled List Entries\\[" + j.toString() + "]\\LVLO\\Reference");
						}
					}
				}
			}
			...
			patch: function (record, helpers, settings, locals) {
				if(settings.processNPC_) {
					...
					for(i = 0; i < xelib.ElementCount(xelib.GetElement(record, "Items")); i++) {
						...
						//Replace items with poverty LVLI
						if(("LVLI|KEYM|WEAP".includes(xelib.Signature(item))) || (("AMMO|ARMO".includes(xelib.Signature(item))) && (GetValue(record, "Items\\[" + j.toString() + "]\\CNTO\\Count") == "1")) || (editorID.includes("Dummy")) || isInBlacklist(locals.blacklist, editorID)) {
							j++;
						} else {
							let lvliRecord;
							if(!xelib.HasElement(patchFile, "LVLI\\p" + editorID)) {
								lvliRecord = AddPovertyLVLI(patchFile, xelib.GetWinningOverride(item), "NPC_");
							} else {
								lvliRecord = xelib.GetElement(0, xelib.Path(patchFile) + "\\LVLI\\p" + editorID);
							}
							xelib.SetLinksTo(record, lvliRecord, "Items\\[" + j.toString() + "]\\CNTO\\Item");
			...
			patch: function (record, helpers, settings, locals) {
				if(settings.processFLORandTREE) {
					...
					let lvliRecord;
					if(!xelib.HasElement(patchFile, "LVLI\\p" + editorID)) {
						lvliRecord = AddPovertyLVLI(patchFile, xelib.GetWinningOverride(ingredient), "FLOR");
					} else {
						lvliRecord = xelib.GetElement(0, xelib.Path(patchFile) + "\\LVLI\\p" + editorID);
					}
			...
			patch: function (record, helpers, settings, locals) {
				if(settings.processFLORandTREE) {
					...
					let lvliRecord;
					if(!xelib.HasElement(patchFile, "LVLI\\p" + editorID)) {
						lvliRecord = AddPovertyLVLI(patchFile, xelib.GetWinningOverride(ingredient), "TREE");
					} else {
						lvliRecord = xelib.GetElement(0, xelib.Path(patchFile) + "\\LVLI\\p" + editorID);
					}
					xelib.SetLinksTo(record, lvliRecord, "PFIG");
...
function AddPovertyLVLI(file, record, origin) {
	...
	//Special cases
	if(editorID.includes("Gold001") && (editorID.includes("Vendor") || editorID.includes("Merchant"))) {
		editorID = editorID + "_MERCHANT";
	} else if(editorID.includes("SpellTome") || editorID.includes("Scroll")) {
		editorID = editorID + "_SPELL";
	} else if(origin == "NPC_" && getsReferencedByRecordWithSignature(record, "COBJ")) {
		editorID = editorID + "_NPC";
	} else if(origin == "NPC_" && signature == "AMMO") {
		editorID = editorID + "_NPC";
	}
	...
	xelib.AddLeveledEntry(lvli, xelib.GetHexFormID(record, true, false), "1", "1");
	switch(signature) {
		case "ALCH":
			if(origin == "FLOR" || origin == "TREE") {
				xelib.AddElementValue(lvli, "LVLG", "pHarvestFoodFlora");
			} else if(origin == "NPC_") {
				xelib.AddElementValue(lvli, "LVLG", "pHarvestFoodNPC");
			} else if(xelib.GetHexFormID(xelib.GetLinksTo(record, "ENIT\\Sound - Consume"), true, false) == "000B6435") {
